...
static void cmd_en(BaseChannel *chp, int argc, char *argv[]) {
...
static void cmd_dis(BaseChannel *chp, int argc, char *argv[]) {
...
static const ADCConversionGroup adcgrpcfg1 = {
  ...
  0, 0,                         /* CR1, CR2 */
  0,
  ADC_SMPR2_SMP_AN3(ADC_SAMPLE_71P5),
  ADC_SQR1_NUM_CH(1),
  0,                            /* SQR2 */
 ...
 */
static void can_rx_cb(CANDriver *canp, uint8_t fifo) {
  float tilt;

  (void)fifo;
  CANRxFrame crf;
  chSysLockFromIsr();
  canTryReceiveI(&CAND1, &crf);
  chSysUnlockFromIsr();
  tilt = *(float *)&(crf.data32[0]);
  pwm = (int16_t)(tilt * 100);
...
int main(void) {
  ...
  canAddFilter(&CAND1, 123 << 5, 0xFFE0, NULL);
  ...
  while (TRUE) {
    ...
    chprintf(&SERIAL_DRIVER, "PWM: %d VCS: %d\r\n", pwm, vcs_mean);
